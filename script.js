...
// 6. Create a new variable. Loop **backwards** through `keyChars` and store each
// value into the new variable, combining each of the values of `keyChars`
var reversedKeyChars = '';
for (i=keyChars.length; i>0; i--) {
  reversedKeyChars += keyChars[i-1];
}
console.log(reversedKeyChars);

// 7. `parseInt` both the `majorKey` and the new variable created in Step 6,
// ensure that both `majorKey` and the new variable are set to these new parsed values.
...
console.log(reversedKeyChars);
majorKey += reversedKeyChars;
// 9. If the new value of `majorKey` is less than 60, set `majorKey` equal to 14.
// If not, check to see if it is equal to 2930, if it is, set `majorKey` equal to 27.
...
function convertToStringAndMaybeDrop(val){
  val = val.toString();
  if(val.length > 1){
